...
{
	if ($device>0) return "Мобильное устройство"; else return "Стационарный компьютер";
}
...
public function Last_enter($last_enter, $device, $ip, $city, $region, $country, $agent)
{
	if (($last_enter['ipv4']!=$ip) OR ($last_enter['device']!=$device) OR ($last_enter['city']!=$city) OR ($last_enter['region']!=$region) OR ($last_enter['country']!=$country) OR ($last_enter['agent']!=$agent)) { $alert='alert-danger'; $warning="<br>\n<strong>Если это были не Вы - немедленно смените пароль!</strong>";} else $alert='alert-success';
		;
	$last_time=date("d.m.Y H:i:s", $last_enter['last_time']);
